...
const COLORS = {
    ...
    discord:     'indigo',
    minecraft:   'emerald',
    // Duck race events use gold/amber so they read as the same family
    // as the dedicated #duck-race embed states (open=lime, racing=amber,
    // complete=gold) — picking gold for both feed envelopes keeps the
    // feed narrative consistent regardless of which state fired the
    // envelope.
    duck_race:   'amber',
};
...
// =========================================================================
// Community signals (Discord, Minecraft)
//
// Generic messaging by design — no usernames, no realm labels in the
// payload. Two reasons:
//   1) Discord usernames are user-controlled and unverified, which makes
//      them an unmoderated injection vector into a public storefront feed
//      (someone joins with a username that's spam/abuse copy → feed
//      surfaces it). Generic "A new member joined the Discord" carries
//      the same activity signal without that risk.
//   2) Realm structure (java / bedrock_horror / bedrock_creative) is
//      operational detail that doesn't need public exposure.
// =========================================================================

export function broadcastDiscordJoin() {
    safeBroadcast('activity.discord.join', envelope(
        'discord.join',
        'New Discord member',
        'A new member joined the Discord.',
        COLORS.discord,
        '👋',
        {},
    ));
...
export function broadcastMinecraftJoin() {
    safeBroadcast('activity.minecraft.join', envelope(
        'minecraft.join',
        'Minecraft realm join',
        'A buyer joined the Minecraft realm.',
        COLORS.minecraft,
        '⛏️',
        {},
    ));
...
// Duck race
//
// entry_added fires when a NEW unique buyer joins the duck race roster
// (one envelope per buyer per session, not per purchase). The detection
// of "new buyer" happens in server.js by diffing rosterCount across
// consecutive roster.updated events.
//
// winner fires when the operator finalizes the race. The dedicated
// #duck-race embed already shows this in place; this envelope mirrors
// the moment into the homepage Activity Feed so viewers landing on the
// homepage at the end of the night get the narrative beat without
// switching to the Duck Race column.
// =========================================================================

export function broadcastDuckRaceEntryAdded(buyerLabel, rosterCount) {
    safeBroadcast('activity.duck_race.entry_added', envelope(
        'duck_race.entry_added',
        'Duck race entry',
        `${buyerLabel} joined the duck race (${rosterCount} entr${rosterCount === 1 ? 'y' : 'ies'}).`,
        COLORS.duck_race,
        '🦆',
        { buyerLabel, rosterCount },
    ));
}

export function broadcastDuckRaceWinner(winnerLabel, entryCount) {
    safeBroadcast('activity.duck_race.winner', envelope(
        'duck_race.winner',
        'Duck race winner',
        `${winnerLabel} wins tonight's duck race${entryCount ? ` (${entryCount} entries)` : ''}!`,
        COLORS.duck_race,
        '🏆',
        { winnerLabel, entryCount },
    ));
}
