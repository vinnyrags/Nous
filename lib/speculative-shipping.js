...
import { metadataFor as tosMetadataFor } from './tos-acceptance.js';
...
function buildDmText({ name, rateLabel, periodLabel, checkoutUrl }) {
    return [
        `Hey ${name} — quick note from tonight's live show.`,
        '',
        `You bought items we open during our live shows (pulls / packs) that we haven't shipped yet. Your cards are held in our inventory waiting on shipping coverage.`,
        '',
 ...
 *
 * @param {string} email
 * @param {string|null} [discordUserId] — when known (caller already
 *   looked it up to send the DM), used to attach the buyer's original
 *   ToS acceptance audit fields to the shipping Stripe session for
 *   audit-trail completeness.
 */
async function createShippingCheckoutUrl(email, discordUserId = null) {
    const stripe = new Stripe(config.STRIPE_SECRET_KEY);
    const rate = getShippingRate(email);

    const baseMetadata = { source: 'shipping_settlement', ...tosMetadataFor(discordUserId) };
    const params = buildCheckoutSessionParams({
        // 7-day window — speculative settlement links shouldn't linger.
        ...
        customerEmail: email,
        // Deliberately NO receiptEmail here (preserves prior behavior).
        successUrl: `${config.SHOP_URL}/thank-you?session_id={CHECKOUT_SESSION_ID}`,
        cancelUrl: config.SHOP_URL,
        metadata: baseMetadata,
    });
    // Mirror the audit fields onto the PaymentIntent — same pattern
    // server.js's applyTosMetadata uses; disputes are filed against
    // the PI, so this is where the audit needs to be visible.
    params.payment_intent_data = { metadata: baseMetadata };
...
export async function settleSpeculativeBuyers() {
    ...
    for (const row of candidates) {
        ...
        try {
            const checkoutUrl = await createShippingCheckoutUrl(email, link.discord_user_id);
