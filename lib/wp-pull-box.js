 ...
 * for pull-box state (one box at a time, finite slots, atomic claims),
...
/**
 * The currently-open pull box, or null. Includes claimed slot numbers
 * + buyer display labels so the on-stream embed can render the grid
 * without a second round-trip.
 */
export async function getActiveBox() {
    const data = await botFetch('/pull-boxes/active');
    return data?.box || null;
}

/**
 * Open a new pull box. Throws if a box is already open (409 from WP).
 */
export async function createBox({ name, priceCents, totalSlots, stripePriceId = null, discordMessageId = null }) {
