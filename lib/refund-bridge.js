...
import { resolveSessionIdFromCharge, retrieveCharge } from '@itzenzottv/stripe-bridge';
...
export async function chargeSessionId(charge) {
    return resolveSessionIdFromCharge(getStripe(), charge);
}
...
export async function handleDisputeEvent(dispute) {
    ...
    let charge;
    try {
        charge = await retrieveCharge(getStripe(), chargeId);
    } catch (e) {
