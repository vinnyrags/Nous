 ...
 *
 * Tests skip the DB write — they run high-volume broadcast() loops
 * (e.g. queue-broadcaster.test.js fires 150 events to exercise the
 * replay-buffer cap) which would otherwise pollute the prod DB when
 * the deploy hook runs `npm test` against /opt/nous-bot/data.db.
 * The DB write IS still exercised by tests that explicitly mock `db.js`
 * to point at a fresh test DB.
 */
