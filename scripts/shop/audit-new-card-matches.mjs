...
async function main() {
    ...
    for (let i = 0; i < newRows.length; i++) {
        ...
        const h = (row[5] || '').trim();   // F number
        const i_ = (row[6] || '').trim();  // G set name
        const j = (row[7] || '').trim();   // H set code
        const p = (row[13] || '').trim();  // N release date
        const r_ = (row[15] || '').trim(); // P Pokemon TCG API ID
