...
async function main() {
    ...
    //   row[0]=name  row[5]=number  row[6]=set  row[8]=variant  row[12]=image
    ...
    rows.forEach((row, i) => {
        ...
        bySetNumber.get(key).push({ rowNum, name: row[0], number, set, image: row[12] || '' });
    });
    ...
    for (const card of enriched) {
        ...
        updates.push({
            range: `${SHEET_NAME}!M${target.rowNum}`,
