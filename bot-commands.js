...
const messages = [
    ...
    // Message 5: Card Shop
    {
        ...
        description: [
            ...
            '',
            '**`/pull open args:"Name" <slots>`** — Open a pull box in `#card-shop`. Posts a Buy Pull button. Price comes from the configured Stripe price ID (`pb_price_id` in shop settings).',
            '> Example: `/pull open args:"Mystery Pull Box" 50`',
            '',
            '**`/pull close`** — Close the active pull box. Single-box model — no tier disambiguation.',
            '',
            '**`/pull status`** — Show active pull box info (slots claimed, total).',
            '',
            '**`/pull replenish args:<count>`** — Add more slots to the active pull box.',
            '',
