...
const messages = [
    ...
    // Message: Test Suite
    {
        title: '🧪 Test Suite',
        description: [
            '**`!test`** — Run both critical path test flows (card night + giveaway/spin). All output routes to #test-suite. Ends with `!reset`.',
            '',
            '**`!test card-night`** — Run card night flow only (73 steps: hype → live → sells → lists → pulls → battles → coupons → queue → duck race → analytics → shipping → offline → dropped-off → reset).',
            '',
            '**`!test giveaway`** — Run giveaway & spin flow only (16 steps: standard giveaway → social giveaway → cancel → all spin variants → reset).',
            '',
            '*Uses @rhapttv test account. Simulates buyer interactions (button clicks, purchases, queue entries) via direct handler calls. Stripe must be in test mode.*',
        ].join('\n'),
        color: 0xceff00,
    },
];
